...
// Python. It exposes the pure-Rust functions from the `mathops` crate to
// Python via PyO3. The compiled result is importable as `pyrust_template._core`.
...
/// returned by `mathops::dot` becomes a Python `ValueError`.
#[pyfunction]
fn dot(a: Vec<f64>, b: Vec<f64>) -> PyResult<f64> {
    mathops::dot(&a, &b).map_err(PyValueError::new_err)
}

/// Return the running cumulative sum of a sequence of floats.
#[pyfunction]
fn cumulative_sum(values: Vec<f64>) -> Vec<f64> {
    mathops::cumulative_sum(&values)
}

// --- Example class ---------------------------------------------------------

/// A running total you can keep adding to. A thin `#[pyclass]` wrapper around
/// the plain-Rust `mathops::Accumulator`, to show stateful objects.
#[pyclass]
struct Accumulator {
    inner: mathops::Accumulator,
}
...
impl Accumulator {
    ...
    fn new(start: f64) -> Self {
        Accumulator {
            inner: mathops::Accumulator::new(start),
        }
    }
